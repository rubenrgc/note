...
function getFragmentQueryDocument(document, fragmentName) {
    ...
    document.definitions.forEach(function (definition) {
        if (definition.kind === 'OperationDefinition') {
            throw process.env.NODE_ENV === "production" ? new invariant.InvariantError(46) : new invariant.InvariantError("Found a " + definition.operation + " operation" + (definition.name ? " named '" + definition.name.value + "'" : '') + ". " +
    ...
    if (typeof actualFragmentName === 'undefined') {
        process.env.NODE_ENV === "production" ? invariant.invariant(fragments.length === 1, 47) : invariant.invariant(fragments.length === 1, "Found " + fragments.length + " fragments. `fragmentName` must be provided when there is not exactly 1 fragment.");
...
function getFragmentFromSelection(selection, fragmentMap) {
    switch (selection.kind) {
        ...
        case 'FragmentSpread': {
            ...
            process.env.NODE_ENV === "production" ? invariant.invariant(fragment, 48) : invariant.invariant(fragment, "No fragment named " + selection.name.value + ".");
            return fragment;
        }
        default:
            return null;
...
function valueToObjectRepresentation(argObj, name, value, variables) {
    if (isIntValue(value) || isFloatValue(value)) {
        ...
        throw process.env.NODE_ENV === "production" ? new invariant.InvariantError(37) : new invariant.InvariantError("The inline argument \"" + name.value + "\" of kind \"" + value.kind + "\"" +
...
function checkDocument(doc) {
    process.env.NODE_ENV === "production" ? invariant.invariant(doc && doc.kind === 'Document', 15) : invariant.invariant(doc && doc.kind === 'Document', "Expecting a parsed GraphQL document. Perhaps you need to wrap the query string in a \"gql\" tag? http://docs.apollostack.com/apollo-client/core.html#gql");
    var operations = doc.definitions
        .filter(function (d) { return d.kind !== 'FragmentDefinition'; })
        .map(function (definition) {
        if (definition.kind !== 'OperationDefinition') {
            throw process.env.NODE_ENV === "production" ? new invariant.InvariantError(16) : new invariant.InvariantError("Schema type definitions not allowed in queries. Found: \"" + definition.kind + "\"");
        }
        return definition;
    });
    process.env.NODE_ENV === "production" ? invariant.invariant(operations.length <= 1, 17) : invariant.invariant(operations.length <= 1, "Ambiguous GraphQL document: contains " + operations.length + " operations");
    return doc;
}
...
function getQueryDefinition(doc) {
    ...
    process.env.NODE_ENV === "production" ? invariant.invariant(queryDef && queryDef.operation === 'query', 18) : invariant.invariant(queryDef && queryDef.operation === 'query', 'Must contain a query definition.');
    return queryDef;
}
function getFragmentDefinition(doc) {
    process.env.NODE_ENV === "production" ? invariant.invariant(doc.kind === 'Document', 19) : invariant.invariant(doc.kind === 'Document', "Expecting a parsed GraphQL document. Perhaps you need to wrap the query string in a \"gql\" tag? http://docs.apollostack.com/apollo-client/core.html#gql");
    process.env.NODE_ENV === "production" ? invariant.invariant(doc.definitions.length <= 1, 20) : invariant.invariant(doc.definitions.length <= 1, 'Fragment must have exactly one definition.');
    var fragmentDef = doc.definitions[0];
    process.env.NODE_ENV === "production" ? invariant.invariant(fragmentDef.kind === 'FragmentDefinition', 21) : invariant.invariant(fragmentDef.kind === 'FragmentDefinition', 'Must be a fragment definition.');
    return fragmentDef;
}
function getMainDefinition(queryDoc) {
    ...
    throw process.env.NODE_ENV === "production" ? new invariant.InvariantError(22) : new invariant.InvariantError('Expected a parsed GraphQL query with a query, mutation, subscription, or a fragment.');
}
...
function validateOperation(operation) {
    ...
    for (var _i = 0, _a = Object.keys(operation); _i < _a.length; _i++) {
        ...
        if (OPERATION_FIELDS.indexOf(key) < 0) {
            throw process.env.NODE_ENV === "production" ? new invariant.InvariantError(36) : new invariant.InvariantError("illegal argument: " + key);
        }
    }
    return operation;
}
...
var ApolloLink = (function () {
    ...
    ApolloLink.prototype.request = function (operation, forward) {
        throw process.env.NODE_ENV === "production" ? new invariant.InvariantError(12) : new invariant.InvariantError('request is not implemented');
    };
...
function assertNotCacheFirstOrOnly(obsQuery) {
    ...
    process.env.NODE_ENV === "production" ? invariant.invariant(fetchPolicy !== 'cache-first' && fetchPolicy !== 'cache-only', 4) : invariant.invariant(fetchPolicy !== 'cache-first' && fetchPolicy !== 'cache-only', 'Queries that specify the cache-first and cache-only fetchPolicies cannot also be polling queries.');
}
...
var serializeFetchParameter = function (p, label) {
    ...
    try {
        ...
        var parseError = process.env.NODE_ENV === "production" ? new invariant.InvariantError(13) : new invariant.InvariantError("Network request failed. " + label + " is not serializable: " + e.message);
...
var checkFetcher = function (fetcher) {
    if (!fetcher && typeof fetch === 'undefined') {
        ...
        if (typeof window === 'undefined')
            library = 'node-fetch';
        throw process.env.NODE_ENV === "production" ? new invariant.InvariantError(14) : new invariant.InvariantError('"fetch" has not been found globally and no fetcher has been ' +
...
var LocalState = (function () {
    ...
    LocalState.prototype.clientQuery = function (document) {
        if (hasDirectives(['client'], document)) {
            ...
            process.env.NODE_ENV === "production" || invariant.invariant.warn('Found @client directives in a query but no ApolloClient resolvers ' +
    ...
    LocalState.prototype.prepareContext = function (context) {
        ...
        var newContext = tslib.__assign(tslib.__assign({}, context), { cache: cache, getCacheKey: function (obj) {
                if (cache.config) {
                    ...
                    process.env.NODE_ENV === "production" ? invariant.invariant(false, 34) : invariant.invariant(false, 'To use context.getCacheKey, you need to use a cache that has ' +
                ...
                execute = function (selection) { return tslib.__awaiter(_this, void 0, void 0, function () {
                    ...
                    return tslib.__generator(this, function (_a) {
                        ...
                        if (isInlineFragment(selection)) {
                            ...
                            process.env.NODE_ENV === "production" ? invariant.invariant(fragment, 35) : invariant.invariant(fragment, "No fragment named " + selection.name.value);
                        }
...
var QueryStore = (function () {
    ...
    QueryStore.prototype.initQuery = function (query) {
        ...
        process.env.NODE_ENV === "production" ? invariant.invariant(!previousQuery ||
            previousQuery.document === query.document ||
            equality.equal(previousQuery.document, query.document), 41) : invariant.invariant(!previousQuery ||
            previousQuery.document === query.document ||
...
var QueryManager = (function () {
    ...
    QueryManager.prototype.stop = function () {
        ...
        this.fetchQueryRejectFns.forEach(function (reject) {
            reject(process.env.NODE_ENV === "production" ? new invariant.InvariantError(23) : new invariant.InvariantError('QueryManager stopped while query was in flight'));
        });
    };
        ...
        function invoke(method, argument) {
            if (observer[method]) {
                try {
                    ...
                    process.env.NODE_ENV === "production" || invariant.invariant.error(e);
                }
            }
            else if (method === 'error') {
                process.env.NODE_ENV === "production" || invariant.invariant.error(argument);
            }
        }
    ...
    QueryManager.prototype.watchQuery = function (options, shouldSubscribe) {
        ...
        process.env.NODE_ENV === "production" ? invariant.invariant(options.fetchPolicy !== 'standby', 26) : invariant.invariant(options.fetchPolicy !== 'standby', 'client.watchQuery cannot be called with fetchPolicy set to "standby"');
    ...
    QueryManager.prototype.query = function (options) {
        var _this = this;
        process.env.NODE_ENV === "production" ? invariant.invariant(options.query, 27) : invariant.invariant(options.query, 'query option is required. You must specify your GraphQL document ' +
            'in the query option.');
        process.env.NODE_ENV === "production" ? invariant.invariant(options.query.kind === 'Document', 28) : invariant.invariant(options.query.kind === 'Document', 'You must wrap the query string in a "gql" tag.');
        process.env.NODE_ENV === "production" ? invariant.invariant(!options.returnPartialData, 29) : invariant.invariant(!options.returnPartialData, 'returnPartialData option only supported on watchQuery.');
        process.env.NODE_ENV === "production" ? invariant.invariant(!options.pollInterval, 30) : invariant.invariant(!options.pollInterval, 'pollInterval option only supported on watchQuery.');
        return new Promise(function (resolve, reject) {
    ...
    QueryManager.prototype.clearStore = function () {
        this.fetchQueryRejectFns.forEach(function (reject) {
            reject(process.env.NODE_ENV === "production" ? new invariant.InvariantError(31) : new invariant.InvariantError('Store reset while query was in flight (not completed in link chain)'));
        });
        var resetIds = [];
    ...
    QueryManager.prototype.getQueryWithPreviousResult = function (queryIdOrObservable) {
        ...
        if (typeof queryIdOrObservable === 'string') {
            ...
            process.env.NODE_ENV === "production" ? invariant.invariant(foundObservableQuery, 32) : invariant.invariant(foundObservableQuery, "ObservableQuery with this id doesn't exist: " + queryIdOrObservable);
    ...
    QueryManager.prototype.startPollingQuery = function (options, queryId, listener) {
        ...
        process.env.NODE_ENV === "production" ? invariant.invariant(pollInterval, 33) : invariant.invariant(pollInterval, 'Attempted to start a polling query without a polling interval.');
        if (!this.ssrMode) {
...
var ApolloClient = (function () {
    function ApolloClient(options) {
        ...
        if (!link || !cache) {
            throw process.env.NODE_ENV === "production" ? new invariant.InvariantError(1) : new invariant.InvariantError("To initialize Apollo Client, you must specify 'uri' or 'link' and " +
...
var justTypenameQuery = gql(templateObject_1 || (templateObject_1 = tslib.__makeTemplateObject(["query { __typename }"], ["query { __typename }"])));
...
var Policies = (function () {
    ...
    Policies.prototype.setRootTypename = function (which, typename) {
        ...
        if (typename !== old) {
            process.env.NODE_ENV === "production" ? invariant__default(old === which, 38) : invariant__default(old === which, "Cannot change root " + which + " __typename more than once");
    ...
    Policies.prototype.readField = function (objectOrReference, nameOrField, getFieldValue, variables, typename) {
        ...
        process.env.NODE_ENV === "production" ? invariant__default(objectOrReference, 39) : invariant__default(objectOrReference, "Must provide an object or Reference when calling Policies#readField");
        var policies = this;
...
function computeKeyObject(response, specifier, aliasMap) {
    ...
    specifier.forEach(function (s) {
        if (Array.isArray(s)) {
            ...
            process.env.NODE_ENV === "production" ? invariant__default(hasOwn.call(response, responseName), 40) : invariant__default(hasOwn.call(response, responseName), "Missing field " + responseName + " while computing key fields");
    ...
    EntityStore.prototype.delete = function (dataId, fieldName) {
        ...
        if (storeObject) {
            ...
            if (storeNamesToDelete_1.length) {
                ...
                if (fieldName) {
                    var cleaned_1 = this.data[dataId] = tslib.__assign({}, storeObject);
...
var storeObjectReconciler = function (existingObject, incomingObject, property, store) {
    ...
    if (existing !== incoming &&
        ...
        process.env.NODE_ENV === "production" ? invariant.invariant(!isReference(existing) || isReference(incoming), 54) : invariant.invariant(!isReference(existing) || isReference(incoming), "Store error: the application attempted to write an object with no provided id but the store already contains an id of " + existing.__ref + " for this object.");
    ...
    StoreReader.prototype.diffQueryAgainstStore = function (_a) {
        ...
        if (hasMissingFields && !returnPartialData) {
            execResult.missing.forEach(function (info) {
                throw process.env.NODE_ENV === "production" ? new invariant.InvariantError(51) : new invariant.InvariantError("Can't find field " + info.fieldName + " on object " + JSON.stringify(info.object, null, 2) + ".");
            });
        }
        return {
        ...
        selectionSet.selections.forEach(function (selection) {
            ...
            if (isField(selection)) {
                ...
                if (isInlineFragment(selection)) {
                    ...
                    process.env.NODE_ENV === "production" ? invariant.invariant(fragment = fragmentMap[selection.name.value], 52) : invariant.invariant(fragment = fragmentMap[selection.name.value], "No fragment named " + selection.name.value);
                }
    ...
    if (!field.selectionSet) {
        ...
        workSet_1.forEach(function (value) {
            if (value && typeof value === "object") {
                process.env.NODE_ENV === "production" ? invariant.invariant(!isReference(value), 53) : invariant.invariant(!isReference(value), "Missing selection set for object of type " + getTypenameFromStoreObject(store, value) + " returned for query field " + field.name.value);
        ...
        selectionSet.selections.forEach(function (selection) {
            ...
            if (isField(selection)) {
                ...
                if (typeof value !== 'undefined') {
                    ...
                    process.env.NODE_ENV === "production" || invariant.invariant.warn("Missing field " + resultFieldKey + " in " + JSON.stringify(result, null, 2).substring(0, 100));
                }
            }
            else {
...
var contextSymbol = Symbol.for('__APOLLO_CONTEXT__');
...
var ApolloProvider = function (_a) {
    ...
    return React.createElement(ApolloContext.Consumer, null, function (context) {
        ...
        process.env.NODE_ENV === "production" ? invariant.invariant(context.client, 5) : invariant.invariant(context.client, 'ApolloProvider was not passed a client instance. Make ' +
...
var ApolloConsumer = function (props) {
    ...
    var ApolloContext = getApolloContext();
    return React.createElement(ApolloContext.Consumer, null, function (context) {
        process.env.NODE_ENV === "production" ? invariant.invariant(context && context.client, 6) : invariant.invariant(context && context.client, 'Could not find "client" in the context of ApolloConsumer. ' +
...
function parser(document) {
    ...
    process.env.NODE_ENV === "production" ? invariant.invariant(!!document && !!document.kind, 8) : invariant.invariant(!!document && !!document.kind, "Argument of " + document + " passed to parser was not a valid GraphQL " +
    ...
    process.env.NODE_ENV === "production" ? invariant.invariant(!fragments.length ||
        (queries.length || mutations.length || subscriptions.length), 9) : invariant.invariant(!fragments.length ||
        (queries.length || mutations.length || subscriptions.length), "Passing only a fragment to 'graphql' is not yet supported. " +
        "You must include a query, subscription or mutation as well");
    process.env.NODE_ENV === "production" ? invariant.invariant(queries.length + mutations.length + subscriptions.length <= 1, 10) : invariant.invariant(queries.length + mutations.length + subscriptions.length <= 1, "react-apollo only supports a query, subscription, or a mutation per HOC. " +
    ...
    var definitions = queries.length
        ...
        : mutations.length
            ? mutations
            : subscriptions;
    process.env.NODE_ENV === "production" ? invariant.invariant(definitions.length === 1, 11) : invariant.invariant(definitions.length === 1, "react-apollo only supports one definition per HOC. " + document + " had " +
...
var OperationData = (function () {
    ...
    OperationData.prototype.refreshClient = function () {
        ...
        process.env.NODE_ENV === "production" ? invariant.invariant(!!client, 49) : invariant.invariant(!!client, 'Could not find "client" in the context or passed in as an option. ' +
    ...
    OperationData.prototype.verifyDocumentType = function (document, type) {
        ...
        process.env.NODE_ENV === "production" ? invariant.invariant(operation.type === type, 50) : invariant.invariant(operation.type === type, "Running a " + requiredOperationName + " requires a graphql " +
...
var QueryData = (function (_super) {
    ...
    QueryData.prototype.prepareObservableQueryOptions = function () {
        ...
        return tslib.__assign(tslib.__assign({}, options), { displayName: displayName, context: options.context, metadata: { reactComponent: { displayName: displayName } } });
    };
    QueryData.prototype.initializeObservableQuery = function () {
        ...
        if (!this.currentObservable.query) {
            ...
            this.currentObservable.query = this.refreshClient().client.watchQuery(tslib.__assign({}, observableQueryOptions));
            if (this.context && this.context.renderPromises) {
...
function useApolloClient() {
    ...
    process.env.NODE_ENV === "production" ? invariant.invariant(client, 7) : invariant.invariant(client, 'No Apollo Client instance can be found. Please ensure that you ' +
